...
def GeneratingParameters(date, code):
    dataX = np.loadtxt("D:/newstock/" + date + "/logicX/" + code + ".txt")
    dataY = np.loadtxt("D:/newstock/" + date + "/logicY/" + code + ".txt")
    m, n = dataX.shape
    ...
    while step < 4_0000:
        ...
        if step % 5000 == 0:
            ...
            np.savetxt("D:/newstock/" + date + "/param/" + code + ".txt", P)
